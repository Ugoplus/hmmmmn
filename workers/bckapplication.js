...
transporter.verify((error, success) => {
  if (error) {
    logger.error('Email configuration failed', { error: error.message });
  } else {
...
function extractUserInfo(cvText, identifier) {
  try {
    // Multiple flexible name extraction patterns
    const namePatterns = [
      /(?:name|full name|applicant name)[:\s]*([A-Za-z][A-Za-z\s\-'.]{1,50})/i,
      /^([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)$/m,
      /^((?:Dr\.?\s+|Engr\.?\s+|Mr\.?\s+|Mrs\.?\s+|Ms\.?\s+)?[A-Z][a-z]+(?:\s+[A-Z][a-z\-']+)*(?:\s+(?:Jr\.?|Sr\.?|II|III))?)$/m,
      /^([A-Z]{2,}\s+[A-Z]{2,}(?:\s+[A-Z]{2,})?)$/m,
      /^([A-Z][a-z]+(?:[\s\-][A-Z][a-z]+)+)/m,
      /\b([A-Z][a-z]{1,15}(?:\s+[A-Z][a-z]{1,15}){0,3})\b/
    ];
    
    let extractedName = '';
    const firstFewLines = cvText.split('\n').slice(0, 5).join('\n');
...
function validateUserInfo(userInfo, identifier) {
  ...
  // Accept if we have email OR phone
  const isValid = validEmail || validPhone;
  
  // Create usable name regardless of extraction success
  if (!userInfo.name || userInfo.name.length < 2) {
    userInfo.name = `Applicant ${identifier.slice(-4)}`;
  }
